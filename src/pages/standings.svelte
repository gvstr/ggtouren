<script>
    ...
    tours.forEach((tour, index) => {
      ...
      tour.competitions.forEach(competition => {
        competition.players.forEach(player => {
            ...
            if(!playerStats.find( p => p.name===player.name )) playerStats.push({name: player.name, points:[], sum: null})
  ...
  function setSum(array){
    array.forEach(x => {
      x.sum = x.points.reduce(function(a, b){ return a + b; }, 0);
    })
    return array;
  }
  ...
  function removeSmallestValues(array, numberOfValuesToRemove, property){
    ...
    temporaryArray.forEach(x => {
      ...
      x[property] = x[property].sort((a, b) => a - b).slice(numberOfValuesToRemove)
    });
...
<div class="container">
  ...
  <div class="grid">
    ...
    <span class="tour-header" on:click={() => toggleHeadlineId(tourIndex)}>{tr.name}</span>
    <span class:hidden={rowIds[tourIndex].hidden} class="competition-header">Plats</span>
    <span class:hidden={rowIds[tourIndex].hidden} class="competition-header">Spelare</span>
    <span class:hidden={rowIds[tourIndex].hidden} class="competition-header">Poäng</span>
      {#each tr.result as r, resultIndex}
        <span class:hidden={rowIds[tourIndex].hidden} class="competition-row">{resultIndex+1}</span>
